...
app.post("/api/generate-script", async (c) => {
  try {
    ...
    if (!openRouterRes.ok) {
      ...
      return c.json({ error: `OpenRouter error: ${err}` }, openRouterRes.status);
    }
...
app.post("/api/generate-image", async (c) => {
  try {
    ...
    if (backend === "huggingface") {
      ...
      if (!hfRes.ok) {
        return c.json({ error: await hfRes.text() }, hfRes.status);
      }
    ...
    if (backend === "replicate") {
      ...
      const repRes = await fetch("https://api.replicate.com/v1/predictions", {
        ...
        body: JSON.stringify({
          version: repModel.includes(":") ? repModel.split(":")[1] : void 0,
          model: !repModel.includes(":") ? repModel : void 0,
...
// .wrangler/tmp/bundle-ZSh7Wm/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZSh7Wm/middleware-loader.entry.ts
